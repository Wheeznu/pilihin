...
class ActorPage {
    ...
    async _init() {
        try {
            ...
            console.warn("ActorPage: retrying...", err);
    ...
    async _findActor() {
        ...
        if (db?.actors?.length) {
            return db.actors.find((a) => a.name.toLowerCase() === name.toLowerCase()) || null;
        }

        try {
            const res = await fetch("/data/data-actor.json");
            const data = await res.json();
            return (data.actors || []).find((a) => a.name.toLowerCase() === name.toLowerCase()) || null;
        } catch {
            return null;
        }
    }
    ...
    _loadFilms() {
        ...
        this._films = db.films.filter(
            (f) => f.status === "published" && f.actors?.some((a) => a.toLowerCase() === this._actor.name.toLowerCase())
        );
    ...
    _getActorFavKey() {
        return "pilih-in-favorites-actors";
    }

    _getActorFavIds() {
        const raw = localStorage.getItem(this._getActorFavKey());
        return raw ? JSON.parse(raw) : [];
    }

    _setActorFavIds(ids) {
        localStorage.setItem(this._getActorFavKey(), JSON.stringify(ids));
    }

    _isActorFavorite() {
        return this._getActorFavIds().includes(this._actor.name);
    }

    _toggleActorFavorite() {
        const ids = this._getActorFavIds();
        const idx = ids.indexOf(this._actor.name);
        if (idx > -1) {
            ids.splice(idx, 1);
        } else {
            ids.push(this._actor.name);
        }
        this._setActorFavIds(ids);
    }
            ...
            <div class="profile-page">
                <div class="profile-header">
                    <div class="profile-photo">
                        <img src="${a.photo || "https://ui-avatars.com/api/?name=${encodeURIComponent(a.name)}&background=1db954&color=fff&size=300"}" alt="${a.name}" onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(a.name)}&background=1db954&color=fff&size=300'" />
                    </div>
                    <div class="profile-info">
                        <div class="profile-name-row">
                            <h1 class="profile-name">${a.name}</h1>
                            <button class="btn-icon btn-fav-profile ${isFav ? "active" : ""}" data-profile="${a.name}" aria-label="Favorit">
                                <i data-feather="heart"></i>
                            </button>
                        </div>
                        <p class="profile-bio">${a.biography || "Tidak ada biografi."}</p>
                    </div>
                </div>
    ...
    _renderError() {
        ...
        container.innerHTML = `
            <div class="detail-error">
                ...
                <h2>Aktor tidak ditemukan</h2>
                <p>Silakan cari aktor dari halaman detail film.</p>
                <a href="/frontend/pages/film/katalog.html" class="btn btn-primary" style="margin-top:var(--space-lg)">
