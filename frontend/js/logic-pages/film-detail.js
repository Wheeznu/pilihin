...
class DetailPage {
    ...
    _bindWatchHistory(video) {
        ...
        const upsert = (progress, completed) => {
            if (!this._db.watchHistory) this._db.watchHistory = [];
            const records = this._db.watchHistory;
            ...
            record.progress = Math.max(record.progress || 0, Math.round(progress));
    ...
    _populateBadges() {
        ...
        container.innerHTML = `
            ...
            ${genreNames.map((g) => `<span class="badge badge--genre">${g.name}</span>`).join("")}
        `;
    }
    ...
    _populateReviews() {
        ...
        const reviews = (this._db?.reviews || []).filter((r) => r.filmId === this._film.id);
        ...
        if (isLoggedIn) {
            ...
            if (submitBtn && input) {
                submitBtn.addEventListener("click", () => {
                    ...
                    if (!this._db.reviews) this._db.reviews = [];
                    this._db.reviews.push({
                        filmId: this._film.id,
    ...
    _isInWatchlist() {
        ...
        const records = this._db?.watchLists || [];
        return records.some((r) => r.userId === user.userId && r.filmId === this._film.id);
    }
    ...
    _bindActions() {
        ...
        if (wlBtn) {
            wlBtn.addEventListener("click", () => {
                ...
                if (!this._db.watchLists) this._db.watchLists = [];
                const idx = this._db.watchLists.findIndex(
                    (r) => r.userId === user.userId && r.filmId === this._film.id
                );
                if (idx > -1) {
                    this._db.watchLists.splice(idx, 1);
                    wlBtn.classList.remove("active-watchlist");
                } else {
                    this._db.watchLists.push({ userId: user.userId, filmId: this._film.id });
                    wlBtn.classList.add("active-watchlist");
                }
                this._saveDb();
            });
        ...
        if (favBtn) {
            favBtn.addEventListener("click", () => {
                ...
                if (!this._db.favorites) this._db.favorites = [];
                const idx = this._db.favorites.findIndex(
                    (r) => r.userId === user.userId && r.filmId === this._film.id
                );
                if (idx > -1) {
                    ...
                    this._db.favorites.push({ userId: user.userId, filmId: this._film.id });
                    favBtn.classList.add("active-favorit");
                }
                this._saveDb();
            });
    ...
    /* ── Error ── */
    _renderError() {
        const container = DOM.$(".detail-page");
        if (!container) return;
        container.innerHTML = `
            <div class="detail-error">
                <i data-feather="film"></i>
                <h2>Film tidak ditemukan</h2>
                <p>Film yang Anda cari tidak tersedia atau telah dihapus.</p>
